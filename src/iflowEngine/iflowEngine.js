  ...
  async calliFlow(idea, taskId) {
    return new Promise((resolve, reject) => {
      ...
      (async () => {
        ...
        let session = null;
        const appIdForSession = idea && typeof idea.ideaKey === 'string' ? this.ideaKeyToAppId.get(idea.ideaKey) || `temp_${taskId}` : `temp_${taskId}`;
        ...
        try {
          if (idea && typeof idea.ideaKey === 'string' && idea.ideaKey) {
            ...
            if (existingAppId) {
              const currentStatus = this.activeApps.get(existingAppId)?.status;
              if (currentStatus === 'running') {
                logger.info(`应用 ${existingAppId} 正在运行，将在迭代完成后自动重启`, { taskId });
