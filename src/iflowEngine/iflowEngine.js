...
class iFlowEngine extends EventEmitter {
  ...
  buildIFlowPrompt({ appId, appDir, port, appType, appName, combinedIdeaText, isNewApp }) {
    return [
      ...
      '',
      '【前端样式要求（必须遵守）】',
      '- 使用原生 CSS 编写样式，禁止使用 Tailwind CSS 或其他 CSS 框架',
      '- 设计风格：简约、高级、现代感，注重细节和微动画',
      '- 配色要求：根据任务主题选择和谐统一的配色方案',
      '  - 如果任务偏商务/文档类：使用白灰黑极简配色',
      '  - 如果任务偏创意/艺术类：使用渐变色或高饱和色彩',
      '  - 如果任务偏科技/数据类：使用深色主题 + 霓虹/赛博风格',
      '- 交互细节：按钮悬停效果、输入框聚焦动画、卡片阴影过渡等',
