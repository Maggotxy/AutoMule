...
const AI_PROMPT_TEMPLATES = [
    // MD风格博客/文档
    '创建一个极简风格的 Markdown 博客首页，白灰黑配色，支持文章列表展示，具有现代感的排版设计',
    '做一个高级感的个人作品集页面，采用黑白灰配色方案，卡片式布局，hover 有流畅动画效果',
    '设计一个技术文档展示页面，左侧目录导航，右侧内容区，深色模式，代码高亮显示',

    // 像素风/复古风
    '创建一个像素风格的个人主页，8-bit 复古配色，像素字体，有趣的像素动画效果',
    '做一个复古游戏风格的计时器应用，像素艺术风格，霓虹色彩，带音效反馈',
    '设计一个像素风格的天气展示卡片，可爱的像素图标，简洁的天气信息展示',

    // 仪表盘/数据展示
    '创建一个现代风格的数据仪表盘，深色主题，渐变色卡片，模拟数据展示，响应式布局',
    '做一个系统状态监控面板，实时数据模拟，进度条和图表展示，科技感 UI 设计',
    '设计一个简约的统计数据展示页，数字滚动动画，图标配合，浅色清爽风格',

    // 工具类应用
    '创建一个简洁的番茄钟应用，圆形进度条，可自定义时间，完成提示动画',
    '做一个 CSS 渐变色生成器，可视化调节颜色，实时预览，一键复制代码',
    '设计一个随机名言生成器，优雅的卡片展示，一键切换，支持复制分享',

    // 互动展示
    '创建一个 3D 卡片翻转效果展示，鼠标悬停触发，正反面不同内容，流畅过渡动画',
    '做一个粒子背景效果页面，鼠标跟随互动，可调节粒子数量和颜色',
    '设计一个打字机效果展示页，逐字显示文字，光标闪烁，可配置的打字速度',

    // 登录/注册页面
    '创建一个高级感的登录页面，毛玻璃效果背景，悬浮卡片表单，输入框聚焦动画',
    '做一个渐变背景的注册页面，左右分栏布局，表单验证提示，提交按钮加载动画',

    // 着陆页
    '创建一个产品着陆页，Hero 大图区域，特性卡片展示，CTA 按钮，白色简约风格',
    '做一个 SaaS 产品首页，定价卡片对比，客户评价轮播，渐变色按钮'
];
